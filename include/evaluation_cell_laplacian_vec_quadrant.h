    ...
    for (unsigned int cell=0; cell<vector_offsets.size(); ++cell)
      {
        ...
        // x-direction
        for (unsigned int i1=0; i1<(dim>2 ? nn : 1)*mid; ++i1)
          {
            VectorizedArray<Number> xp[mid];
            for (unsigned int i=0; i<mid; ++i)
              {
                const VectorizedArray<Number> x = input_ptr[i1*mid+i];
            ...
            for (unsigned int col=0; col<mid; ++col)
              {
                ...
                data_ptr[i1*mid+col].data = _mm256_permute_pd(r0.data, 0x5) + (one_x * r0).data;
              }
          }
        // y-direction
        ...
        VectorizedArray<Number>* phi_grads = data_ptr + dofs_per_cell_z;
        for (unsigned int q=0; q<n_q_points; ++q)
          {
            if (dim==2)
              {
        ...
        for (unsigned int i1=0; i1<(dim>2 ? nn : 1)*mid; ++i1)
          {
            ...
            for (unsigned int i=0; i<mid; ++i)
              {
                const VectorizedArray<Number> x = data_ptr[i1*mid+i];
                xp[i].data = _mm256_permute_pd(x.data, 0x5) + (one_x * x).data;
              }
            for (unsigned int col=0; col<mid; ++col)
              {
                ...
                output_ptr[i1*mid+col].data = _mm256_permute_pd(r0.data, 0x5) + (one_x * r0).data;
              }
          }
      }
  }
