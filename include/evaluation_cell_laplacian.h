...
public:
  ...
  void
  initialize(const unsigned int n_element_batches, const bool is_cartesian)
  {
        ...
        for (unsigned int i = 0; i < n_element_batches; ++i)
          {
            ...
            for (unsigned int q = 0; q < n_q_points; ++q)
              jxw_data[data_offsets[i] + q] = jacobian_determinant * quadrature_weights[q];
            for (unsigned int q = 0; q < n_q_points; ++q)
              for (unsigned int d = 0; d < dim; ++d)
                jacobian_data[data_offsets[i] * dim * dim + q * dim * dim + d * dim + d] =
                  jacobian[d];
          }
        std::abort();
      }
          ...
          if (dim == 3)
            for (unsigned int q1 = 0; q1 < degree + 1; ++q1)
              for (unsigned int q0 = 0; q0 < degree + 1; ++q0)
                sum += gll.value(i1, gauss_points[q1]) * gll.value(i0, gauss_points[q0]) *
                       gauss_weights[q0] * gauss_weights[q1];
          else if (dim == 2)
            for (unsigned int q0 = 0; q0 < degree + 1; ++q0)
...
#  endif
        ...
        std::cout << "Error of integral in direction " << test << ": " << max_error << std::endl;
      }
#endif
  }
...
#else
    const Number *__restrict shape_values    = this->shape_values.begin();
    const Number *__restrict shape_gradients = this->shape_gradients.begin();
...
#  ifdef DO_FACES
    ...
    VectorizedArray<Number> tmp1[2 * dofs_per_face], tmp2[dim * dofs_per_face],
      tmp3[dim * dofs_per_face], tmp4[dofs_per_face], tmp5[dofs_per_face];
#  endif
...
#  ifdef DO_MASS_MATRIX
        ...
        const VectorizedArray<Number> *__restrict jxw_ptr = jxw_data.begin() + data_offsets[cell];
        if (is_cartesian)
            ...
            for (unsigned int i1 = 0; i1 < nn; ++i1) // loop over y layers
              {
                ...
                for (unsigned int i = 0; i < nn; ++i)
                  {
                    const VectorizedArray<Number> res =
                      in[i1 * nn + i] * quadrature_weights[i2 * nn * nn + i1 * nn + i];
...
#    else
        ...
        if (is_cartesian)
          for (unsigned int d = 0; d < dim; ++d)
            merged_array[d] = jxw_ptr[0] * jacobian_ptr[d] * jacobian_ptr[d];
        ...
        for (unsigned int i2 = 0; i2 < nn_3d; ++i2) // loop over z layers
          {
            ...
            // x-derivative
            VectorizedArray<Number> outx[nn * nn];
            for (unsigned int i1 = 0; i1 < nn; ++i1) // loop over y layers
              {
                apply_1d_matvec_kernel<nn, 1, 1, true, false, VectorizedArray<Number>>(
                  shape_gradients, in + i1 * nn, outx + i1 * nn);
              }
              ...
              for (unsigned int i = 0; i < nn * nn; ++i)
                {
                  const VectorizedArray<Number> weight =
                    make_vectorized_array(quadrature_weights[i2 * nn * nn + i]);
                  if (dim == 2)
                    {
                      VectorizedArray<Number> t0 =
                        outy[i] * merged_array[2] + outx[i] * merged_array[0];
                      VectorizedArray<Number> t1 =
                        outy[i] * merged_array[1] + outx[i] * merged_array[2];
                      outx[i] = t0 * weight;
                      outy[i] = t1 * weight;
                    }
                  else if (dim == 3)
                    {
                      VectorizedArray<Number> t0 = outy[i] * merged_array[3] +
                                                   outz[i2 * nn * nn + i] * merged_array[4] +
                                                   outx[i] * merged_array[0];
                      VectorizedArray<Number> t1 = outy[i] * merged_array[1] +
                                                   outz[i2 * nn * nn + i] * merged_array[5] +
                                                   outx[i] * merged_array[3];
                      VectorizedArray<Number> t2 = outy[i] * merged_array[5] +
                                                   outz[i2 * nn * nn + i] * merged_array[2] +
                                                   outx[i] * merged_array[4];
                      outx[i] = t0 * weight;
                      outy[i] = t1 * weight;
                      outz[i2 * nn * nn + i] = t2 * weight;
                    }
                }
            else
              for (unsigned int i = 0; i < nn * nn; ++i)
                {
                  const unsigned int q = i2 * nn * nn + i;
                  const VectorizedArray<Number> *geo_ptr = jacobian_ptr + q * dim * dim;
                  VectorizedArray<Number> grad[dim];
                  // get gradient
                  for (unsigned int d = 0; d < dim; ++d)
                    {
                      grad[d] = geo_ptr[d * dim + 1] * outy[i];
                      if (dim == 3)
                  ...
                  if (dim == 3)
                    {
                      ...
                      outy[i] = jxw_ptr[q] * (geo_ptr[1] * grad[0] + geo_ptr[1 + dim] * grad[1] +
                ...
                for (unsigned int i1 = 0; i1 < nn; ++i1) // loop over y layers
                  {
                    apply_1d_matvec_kernel<nn, 1, 1, false, false, VectorizedArray<Number>>(
                      shape_gradients, outx + i1 * nn, in + i1 * nn);
...
#    ifdef DO_CONVECTION
            if (dim == 2)
              {
                ...
                apply_1d_matvec_kernel<nn, 1, 0, true, false, VectorizedArray<Number>>(shape_values,
                                                                                       tmp1,
                                                                                       tmp4);
                for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
            ...
            else
              {
                for (unsigned int i2 = 0; i2 < degree + 1; ++i2)
                  for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                    tmp1[i2 * (degree + 1) + i1] =
                      input_ptr[degree * offset1 + i2 * stride2 + i1 * stride1];
                for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                  apply_1d_matvec_kernel<nn, 1, 0, true, false, VectorizedArray<Number>>(
            ...
            if (dim == 2)
            ...
            else
              {
                ...
                for (unsigned int i2 = 0; i2 < degree + 1; ++i2)
                  for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                    output_ptr[degree * offset1 + i2 * stride2 + i1 * stride1] +=
                      tmp1[i2 * (degree + 1) + i1];
                for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                  apply_1d_matvec_kernel<nn, 1, 0, false, false, VectorizedArray<Number>>(
...
#    else
                ...
                for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                  {
                    tmp1[i1] = input_ptr[i1 * stride1];
                    tmp1[i1 + dofs_per_face] =
                      hermite_derivative_on_face * (input_ptr[offset1 + i1 * stride1] - tmp1[i1]);
                  }
                apply_1d_matvec_kernel<nn, 1, 0, true, false, VectorizedArray<Number>>(
            ...
            else
              {
                  ...
                  for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                    {
                      tmp1[i2 * (degree + 1) + i1] =
                        input_ptr[degree * offset1 + i2 * stride2 + i1 * stride1];
                      tmp1[i2 * (degree + 1) + i1 + dofs_per_face] =
                        hermite_derivative_on_face *
                        (input_ptr[(degree - 1) * offset1 + i2 * stride2 + i1 * stride1] -
                         tmp1[i2 * (degree + 1) + i1]);
                    }
                for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                  apply_1d_matvec_kernel<nn, 1, 0, true, false, VectorizedArray<Number>>(
                    shape_values,
                    tmp1 + dofs_per_face + i1 * (degree + 1),
                    tmp2 + 2 * dofs_per_face + i1 * (degree + 1));
                for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                  apply_1d_matvec_kernel<nn, nn, 0, true, false, VectorizedArray<Number>>(
                  ...
                  for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                    {
                      ...
                      tmp1[i2 * (degree + 1) + i1 + dofs_per_face] =
                        hermite_derivative_on_face *
                        (input_ptr[offset1 + i2 * stride2 + i1 * stride1] -
                         tmp1[i2 * (degree + 1) + i1]);
                    }
                for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                  apply_1d_matvec_kernel<nn, 1, 0, true, false, VectorizedArray<Number>>(
                ...
                for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                  apply_1d_matvec_kernel<nn, 1, 0, true, false, VectorizedArray<Number>>(
                    shape_values,
                    tmp1 + dofs_per_face + i1 * (degree + 1),
                    tmp3 + 2 * dofs_per_face + i1 * (degree + 1));
                for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                  apply_1d_matvec_kernel<nn, nn, 0, true, false, VectorizedArray<Number>>(
            ...
            const VectorizedArray<Number> sigmaF =
              Number((degree + 1) * (degree + 1)) * normal_jac1[f][dim - 1];
            for (unsigned int q = 0; q < Utilities::pow(degree + 1, dim - 1); ++q)
              {
                ...
                average_valgrad = weight * (average_value * 2. * sigmaF - average_valgrad * 0.5);
            ...
            if (dim == 2)
              {
                ...
                for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                  {
                    output_ptr[degree * offset1 + i1 * stride1] +=
                      tmp1[i1] - tmp1[i1 + dofs_per_face] * hermite_derivative_on_face;
                    output_ptr[(degree - 1) * offset1 + i1 * stride1] +=
                      hermite_derivative_on_face * tmp1[i1 + dofs_per_face];
                  }
                ...
                for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                  {
                    output_ptr[i1 * stride1] +=
                      tmp1[i1] - tmp1[i1 + dofs_per_face] * hermite_derivative_on_face;
                    output_ptr[offset1 + i1 * stride1] +=
                      hermite_derivative_on_face * tmp1[i1 + dofs_per_face];
                  }
              }
            else
              {
                  ...
                  for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                    {
                      ...
                      output_ptr[(degree - 1) * offset1 + i2 * stride2 + i1 * stride1] +=
                        hermite_derivative_on_face * tmp1[i2 * (degree + 1) + i1 + dofs_per_face];
                    }
                ...
                for (unsigned int i2 = 0; i2 < degree + 1; ++i2)
                  for (unsigned int i1 = 0; i1 < degree + 1; ++i1)
                    {
                      output_ptr[i2 * stride2 + i1 * stride1] +=
                        tmp1[i2 * (degree + 1) + i1] -
                        tmp1[i2 * (degree + 1) + i1 + dofs_per_face] * hermite_derivative_on_face;
                      output_ptr[offset1 + i2 * stride2 + i1 * stride1] +=
                        hermite_derivative_on_face * tmp1[i2 * (degree + 1) + i1 + dofs_per_face];
                    }
              }
#    endif // !CONVECTION
...
private:
  ...
  fill_shape_values()
  {
    ...
    if (degree % 2 == 0)
      for (unsigned int q = 0; q < stride; ++q)
        shape_values[degree / 2 * stride + q] = basis.value(degree / 2, gauss_points[q]);
    ...
    if (std::abs(hermite_derivative_on_face[0] + basis.derivative(1, 0)) > 1e-12)
      std::cout << "Error, unexpected value of Hermite shape function derivative: "
                << hermite_derivative_on_face[0] << " vs " << basis.derivative(1, 0) << std::endl;
      ...
      for (unsigned int q = 0, z = 0; z < n_q_points_1d; ++z)
        for (unsigned int y = 0; y < n_q_points_1d; ++y)
          for (unsigned int x = 0; x < n_q_points_1d; ++x, ++q)
            quadrature_weights[q] = (gauss_weight_1d[z] * gauss_weight_1d[y]) * gauss_weight_1d[x];
    else if (dim == 2)
